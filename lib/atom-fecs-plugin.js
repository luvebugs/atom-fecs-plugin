...
export default {
    ...
    addDecorators(editor, errors) {
        ...
        for (key in messageMap) {
            ...
            const messages= messageMap[key].map(item => item.message);
            const severity= messageMap[key].every(item => item.severity - 1);
            const decorations = [
                editor.decorateMarker(marker, {
                    'type': 'block',
                    'position': 'after',
                    'item': new MessageView().getElement(messages),
                })
            ].concat(severity === 1 ? [
                editor.decorateMarker(marker, {
                    'type': 'line',
    ...
    updateStatusBar(editor, messageMap) {
        ...
        if (messageMap[line]) {
            this.statusBarView.setInfo(messageMap[line][0].info);
        }
    },
    bindEvent(editor) {
        $(document).on('click', '.icon-sync', () => {
